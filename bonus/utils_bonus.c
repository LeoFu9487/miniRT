...
static double	get_new_brightness(t_objects *obj, t_intersect *it, t_light *light, t_camera *camera)
{
	double	ans;
	...
	len_sum = two_points_distance(it->coordinate, light->coordinate) + two_points_distance(it->coordinate, camera->coordinate);
	len_sum /= LEN_UNIT;
	ans /= len_sum * len_sum;
	assigned_func2((void**)func);
	vector[0] = func[obj->type](it->coordinate, obj->ptr);
	vector[1] = two_points_vector(it->coordinate, camera->coordinate);
...
void		intersect_color_bonus(t_intersect *it, t_parse *parse, t_camera *camera)
{
	...
	while (lst)
	{
		...
		if (have_intersection(parse->objects, line))
			continue ;
		new_brightness = get_new_brightness(obj, it, light, camera);
