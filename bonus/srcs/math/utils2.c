...
double	*reflection_color(int *color, t_light *light, double new_brightness)
{
	...
	ct = -1;
	while (++ct < 3)
		ans[ct] = (double)color[ct] *
		(double)(light->color[ct]) / 255.0 * new_brightness;
	return (ans);
}

double	*flashlight_reflection(int *color,
t_flashlight *flashlight, double new_brightness)
{
	double	*ans;
	int		ct;
	...
	while (++ct < 3)
		ans[ct] = (double)color[ct] *
		(double)(flashlight->color[ct]) / 255.0 * new_brightness;
	return (ans);
}
